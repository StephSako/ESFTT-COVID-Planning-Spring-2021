<?php
    ...
    if(!empty($_POST['jour_debut']) && !empty($_POST['jour_fin'])) {

        $jour_debut = explode('/', $_POST['jour_debut']);
        $jour_debut = $jour_debut[2] . '-' . $jour_debut[1] . '-' . $jour_debut[0];
        $jour_fin = explode('/', $_POST['jour_fin']);
        $jour_fin = $jour_fin[2] . '-' . $jour_fin[1] . '-' . $jour_fin[0];
        ...
        $creneau = new Creneau();
        $creneau->creation($jour_debut, $jour_fin, $horaire_debut, $horaire_fin, $jour);
	}