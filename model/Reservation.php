<?php
	...
	class Reservation {
		...
		public function deja_inscrit($id_creneau){
			$result = mysqli_query($this->co, 'SELECT id_reservation FROM reservations WHERE id_joueur = ' . $this->id_joueur . ' AND id_creneau = ' . $id_creneau);
			return (mysqli_num_rows($result) > 0) ? true : false;
		}

		public function suppression($id_creneau){
			mysqli_query($this->co, 'DELETE FROM reservations WHERE id_joueur = ' . $this->id_joueur . ' AND id_creneau = ' . $id_creneau) or die('Impossible de supprimer la réservation.');
		}

		public function modification($id_creneau, $reservations, $reservations_org){
			mysqli_query($this->co, 'UPDATE reservations SET mardi = ' . $mardi . ', mercredi = ' . $mercredi . ', vendredi = ' . $vendredi . ', dimanche_matin = ' . $dimanche_matin . ', org_mardi = ' . $org_mardi . ', org_mercredi = ' . $org_mercredi . ', org_vendredi = ' . $org_vendredi . ', org_dimanche_matin = ' . $org_dimanche_matin . ' WHERE id_joueur = ' . $this->id_joueur .' AND id_creneau = ' . $id_creneau) or die('Impossible de modifier la réservation.');
		}

		public function inscription($table, $id_joueur, $reservations, $reservations_org, $date){
